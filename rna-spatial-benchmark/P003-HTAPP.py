...
def load_htapp_data(sample_name):
    """
    加载HTAPP数据集
    """
    ...
    sp = sc.read_h5ad(spatial_path)
    return rna, sp


def prepare_spatial_coordinates(spatial_data):
    """
    处理HTAPP数据集的spatial数据
    检查并转换坐标信息格式
    """
    # 检查是否已经有spatial坐标信息
...
def save_final_results(method, all_sample_results, dataset_avg, results_dir="./P003-HTAPP"):
    """
    保存最终结果到JSON文件
    """
    # 创建结果目录
    ...
    # 保存到JSON文件
    filename = f"HTAPP-{method}-final-results.json"
...
def main():
    """
    主函数：执行HTAPP数据集的所有样本测试
    """
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='HTAPP数据集所有样本测试')
    parser.add_argument('--method', type=str, required=True, 
                       choices=['spage', 'tangram', 'gimvi', 'novosparc', 'spaotsc', 'stplus', 'scfugw'],
                       help='要运行的算法名称')
    parser.add_argument('--results_dir', type=str, default='./P003-HTAPP',
                       help='结果保存目录，默认为./P003-HTAPP')
    ...
    for i, sample_name in enumerate(samples):
        ...
        # if sample_name == 'HTAPP-514-SMP-6760' and method == 'scfugw':
        #     print(f"跳过样本 {sample_name}")
        #     continue

        
        # 加载数据
        rna, sp = load_htapp_data(sample_name)
        sp = prepare_spatial_coordinates(sp)
        rna, sp = preprocess_data(rna, sp)
        
        # 加载测试基因 (10-fold交叉验证)
        test_genes_all_folds = np.load(f"../../../DataCenter/HTAPP/test_squidgenes/{sample_name}_test.npy", allow_pickle=True)
        ...
        for fold_idx in range(test_genes_all_folds.shape[0]):
            ...
            if result is not None:
                # 确保结果中的值都是Python float类型
                converted_result = {
                    'PCC': float(result['PCC']),
                    'JS': float(result['JS']),
                    'RMSE': float(result['RMSE']),
                    'SSIM': float(result['SSIM'])
                }
                all_fold_results.append(converted_result)
        ...
        if all_fold_results:
            ...
            for metric in ['PCC', 'JS', 'RMSE', 'SSIM']:
                values = [float(result[metric]) for result in all_fold_results]  # 确保转换为Python float
