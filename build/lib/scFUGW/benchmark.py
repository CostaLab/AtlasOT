...
class Benchmark:
    ...
    def spage(self) -> Optional[pd.DataFrame]:
        ...
        # 运行SpaGE
        res = SpaGE(sp_df[feature], rna_df.T, n_pv=min(100, max(1, len(feature) // 2)), genes_to_predict=predict)
        return res[predict]
    ...
    def gimvi(self, max_epochs: int = 200) -> Optional[pd.DataFrame]:
        ...
        train_present = [g for g in self.train_genes if g in self.sp.var_names]
    ...
    def novosparc(self) -> Optional[pd.DataFrame]:
        ...
        # 计算标记基因成本
        insitu_matrix = np.array(sp_df[feature])
        gene_names = np.array(rna_df.columns.values)
        markers_in_sc = [np.where(gene_names == marker)[0][0] for marker in feature if marker in gene_names]
        if len(markers_in_sc) == 0:
            return None
        
        cost_marker_genes = cdist(dge[:, markers_in_sc] / (np.amax(dge[:, markers_in_sc]) + 1e-8),
                                  insitu_matrix / (np.amax(insitu_matrix) + 1e-8))
    ...
    def spaotsc(self) -> Optional[pd.DataFrame]:
        ...
        rho = np.corrcoef(np.vstack([a.T.values, b.T.values]))
        mcc = rho[-len(b.columns):, :len(a.columns)] if rho.ndim == 2 else np.zeros((b.shape[1], a.shape[1]))
        C = np.exp(1 - mcc)
        
        # 运行SpaOTsc
        issc = SpaOTsc.spatial_sc(sc_data=b, is_data=a, is_dmat=is_dmat)
        ...
        result = pd.DataFrame(data=X_pred, columns=issc.sc_data.columns.values, index=a.index)
