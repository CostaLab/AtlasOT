...
def split_indices(
    ...
    """Split sample indices into batches for mini-batch processing.
...
def compute_batch_cost_matrix(
    adata: AnnData,
    modality: str = 'rna',
    k: int = 30,
    device: torch.device | None = None,
) -> np.ndarray:
    """Compute intra-modality cost matrix for a data batch.

    Automatically selects the appropriate embedding key and cost function
    based on modality ('rna', 'atac', or 'spatial').
    ...
    device : torch.device or None
        Target device (unused, kept for API compatibility).

    Returns
    -------
    np.ndarray
        Normalized cost matrix of shape (n_batch, n_batch).
    """
...
def minibatch_atlasot(
    ...
    """Run AtlasOT in mini-batch mode for memory-efficient large-scale alignment.

    Splits source and target data into batches, computes per-batch cost matrices
    and shared spaces locally, then aggregates results across batches.
    ...
    source_modality : str
        Modality type for source ('rna' or 'atac').
    target_modality : str
        Modality type for target ('rna', 'atac', or 'spatial').
    batch_source : int
    ...
    device : torch.device or None
        Compute device.
    alpha : float
        FUGW feature-term weight.
    rho : float
        FUGW unbalancedness parameter.
    eps : float
        FUGW entropic regularization.
    random_seed : int
        Random seed.
    verbose : bool
        Print progress.
    projection_keys : list of str or None
        Keys in target to project via π (e.g., ['X_pca', 'X']).
    label_columns : list of str or None
        Column names in target.obs for label transfer.
    top_k : int
        K for KNN-based label transfer voting.

    Returns
    -------
    label_results_df : pd.DataFrame or None
        Label transfer results per source cell, or None if label_columns is None.
    projected_results : dict
        Dict mapping projection_key → projected array.
    full_target_features : np.ndarray
        Full target features used for alignment.
    """
    if device is None:
