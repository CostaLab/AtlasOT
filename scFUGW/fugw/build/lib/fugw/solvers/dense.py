...
class FUGWSolver(BaseSolver):
    ...
    def local_biconvex_cost(
        ...
        """
        rho_s, rho_t, eps, alpha, reg_mode, divergence, lambda_laplacian = hyperparams
        ws, wt, ws_dot_wt = tuple_weights
        X_sqr, Y_sqr, X, Y, D, L = data_const
        if transpose:
        ...
        if lambda_laplacian and lambda_laplacian > 0 and L is not None:
            laplacian_grad = 2 * torch.matmul(pi, L)
            cost += lambda_laplacian * laplacian_grad
    ...
    def fugw_loss(
        ...
        loss_laplacian_regularization = torch.zeros(1)
        loss = 0
        ...
        if lambda_laplacian is not None and lambda_laplacian > 0 and lambda_laplacian != float("inf") and L is not None:
            loss_laplacian_regularization = torch.trace(pi @ L @ pi.T)
            loss += lambda_laplacian * loss_laplacian_regularization
        ...
        return {
            ...
            "laplacian_regularization": loss_laplacian_regularization.item(),
            "total": loss.item(),
        }
    ...
    def solve(
        ...
        compute_local_biconvex_cost = partial(
            ...
            data_const=(Ds_sqr, Dt_sqr, Ds, Dt, F, L),
            tuple_weights=(ws, wt, ws_dot_wt),
            hyperparams=(rho_s, rho_t, eps, alpha, reg_mode, divergence, lambda_laplacian), # 记得这里也要改
        )
        ...
        compute_fugw_loss = partial(
            ...
            data_const=(Ds_sqr, Dt_sqr, Ds, Dt, F, L),
            tuple_weights=(ws, wt, ws_dot_wt),
            hyperparams=(rho_s, rho_t, eps, alpha, reg_mode, divergence, lambda_laplacian),
        )

        compute_fugw_loss_validation = partial(
            self.fugw_loss,
            data_const=(Ds_sqr_val, Dt_sqr_val, Ds_val, Dt_val, F_val, L),
            tuple_weights=(ws, wt, ws_dot_wt),
            hyperparams=(rho_s, rho_t, eps, alpha, reg_mode, divergence, lambda_laplacian),
        )
